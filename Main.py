...
def OpcionesAdministrador():
    ...
    if op == 0:
        ...
        print("Las solicitudes que tiene pendientes son: ")
        with open("Textos/SolicitudesPendientes.txt", "r") as archivo:
            for i in archivo:
                print(i)
        
        IdSolicitud = input("Id de la solicitud a aceptar: ")
        state = ""
        with open("Textos/SolicitudesPendientes.txt", "r") as archivo:
            for x in archivo:
                g = x.split()
                ...
                if IdSolicitud in g:
                    ...
                    for i, item in enumerate(g): # Cambiar el estado de la solicitud
                        if item.lower() == "pendiente":
                            g[i] = "Aceptada"
                    g_str = " ".join(g)  # Convertir la lista a string
                    with open(f"Textos/{nombre}.txt", "a") as archivo_nombre:
                        archivo_nombre.write("\n" + g_str)
                    
            print("Solicitud numero " + IdSolicitud + " ha sido aceptada")
